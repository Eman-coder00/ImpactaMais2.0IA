    ...
    try {
        ...
        app.get('/projetos', async (req, res) => {
            try {
                ...
                // Pipeline para buscar posts com dados do autor (foto de perfil, nome)
                const pipeline = [
                    { $match: filter },
                    {
                        $lookup: {
                ...
                if (req.session.user) {
                    ...
                    if (currentUser && currentUser.friends && currentUser.friends.length > 0) {
                        friendsList = await db.collection('users').find({
                            _id: { $in: currentUser.friends.map(id => new ObjectId(id)) }
                        }).toArray();
                    }
                }
                ...
                console.error('Erro ao carregar listagem de projetos:', error);
                res.redirect('/');
            }
        });
        ...
        app.get('/usuario/:id', async (req, res) => {
            try {
                ...
                if (!targetUser) return res.redirect('/projetos');

                const myProjects = await db.collection('posts').find({ authorId: targetId }).toArray();
                const eventIds = targetUser.eventsJoined || [];
                const joinedEvents = await db.collection('events').find({ _id: { $in: eventIds } }).toArray();

                // Status de amizade (se logado)
                ...
                if (req.session.user) {
                    ...
                    if (currentUser.friends && currentUser.friends.some(id => id.toString() === targetId.toString())) {
                        friendshipStatus = 'friends';
                    } else {
                        // Verifica se existe solicitação pendente em qualquer um dos lados
                        const hasSentRequest = targetUser.friendRequests?.some(r => r.fromId.toString() === currentUserId.toString());
                        const hasReceivedRequest = currentUser.friendRequests?.some(r => r.fromId.toString() === targetId.toString());
        ...
        app.post('/amizade/request', async (req, res) => {
            ...
            try {
                ...
                res.redirect(req.get('Referer') || `/usuario/${req.body.targetId}`);
            } catch (error) {
        ...
        async function getConversations(myId) {
            ...
            for (const msg of messages) {
                const friendId = msg.senderId.toString() === myId.toString() ? msg.receiverId : msg.senderId;
                const friendIdStr = friendId.toString();

                if (!conversationMap.has(friendIdStr)) {
                    const friend = await db.collection('users').findOne({ _id: friendId });
                    if (friend) {
                        conversationMap.set(friendIdStr, {
                            ...
                            unreadCount: (msg.receiverId.toString() === myId.toString() && !msg.read) ? 1 : 0
                        });
                    }
                } else if (msg.receiverId.toString() === myId.toString() && !msg.read) {
                    const conv = conversationMap.get(friendIdStr);
                    conv.unreadCount++;
                }
            }
        ...
        app.get('/mensagens', async (req, res) => {
            ...
            try {
                ...
                res.render('mensagens', { conversations });
            } catch (error) {
                console.error('Erro ao carregar lista de mensagens:', error);
                res.redirect('/perfil');
            }
        });
        ...
        app.get('/api/conversations', async (req, res) => {
            ...
            try {
                ...
                console.error('Erro ao carregar API de conversas:', error);
                res.status(500).json({ error: 'Erro interno' });
            }
        });
